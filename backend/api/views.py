...
class CustomUserViewSet(UserViewSet):
    ...
    def get_permissions(self):
        if self.action == 'me' and self.request.user.is_authenticated == False:
            return (OwnerOnly(), )
    ...
    def subscribe(self, request, id=None):
        ...
        serializer = FollowCreateSerializer(data={"user": request.user, "following": following},
    ...
    def subscriptions(self, request):
        ...
        if page is not None:
            serializer = FollowCreateSerializer(page, many=True, context={'request': request})
            return self.get_paginated_response(serializer.data)

        print(following)
        serializer = FollowCreateSerializer(following, many=True, context={'request': request})
...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def shopping_cart(self, request, pk=None):
        if Recipe.objects.filter(pk=pk).exists():
            ...
            serializer.save()
            return Response(status=HTTP_201_CREATED, data=serializer.data)
        return Response(status=HTTP_400_BAD_REQUEST, data={"errors": "Такого рецепта не существует"})
    ...
    def shopping_cart_delete(self, request, pk=None):
        recipe = get_object_or_404(
            Recipe,
            pk=pk)
        if ShoppingList.objects.filter(recipe=recipe, user=request.user).exists():
            instance = ShoppingList.objects.get(recipe=recipe, user=request.user)
            instance.delete()
            return Response(status=HTTP_204_NO_CONTENT)
        return Response(status=HTTP_400_BAD_REQUEST, data={"errors": "Такого рецепта нет в списке покупок"})
    ...
    def favorite_delete(self, request, pk=None):
        ...
        if Favorites.objects.filter(recipe=recipe, user=request.user).exists():
            ...
            instance.delete()
            return Response(status=HTTP_204_NO_CONTENT)
        return Response(status=HTTP_400_BAD_REQUEST, data={"errors": "Такого рецепта нет в избранном"})
